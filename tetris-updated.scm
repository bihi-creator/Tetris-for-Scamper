...
(define spawn-shape!
  (lambda ()
    (ignore (let* ([shape (vector-ref spawnable-shapes 
                   ...
                   [indeces (cdr shape)])
              (begin
                (ref-set! current-shape-vector (car shape))
                (ref-set! shape-index (list-ref indeces (random (length indeces))))
                (ref-set! current-shape-drawing (shape->drawing (deref current-shape-vector)
                                                                (deref shape-index))))))))
...
(define draw-game
  (lambda ()
    (begin 
      ...
      (canvas-drawing! canv (deref x-position) (deref y-position) current-shape-vector))))
...
; Temporary buttons for changing game states
...
(ignore
  (animate-with
    (lambda (time)     
      (let* ([game-time (round (- (round time) (deref state-change)))]
             [height (remainder (+ (remainder (* (quotient (round (- (round time)
                                                                     (deref state-change)))
                                                           750)
                                                 20)
                                              360)
             ...
             [time-for-respawn (remainder (round (- (round time) (deref state-change))) 13500)])
            (begin
              (ref-set! y-position height)
